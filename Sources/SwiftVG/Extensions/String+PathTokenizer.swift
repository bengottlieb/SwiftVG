...
extension String {
	struct PathTokenizer {
		struct PathSegment {
			...
			mutating func addCoordinate(_ coords: [String]) {
				arguments += coords
			}
		...
		var segments: [PathSegment] {
			...
			while true {
				if let next = parser.nextCommand() {
					let argCount = next.argumentCount
					segments.append(PathSegment(command: next, arguments: parser.nextNArguments(argCount), origin: self.origin))
				} else if let last = segments.last {
					if !last.command.willAcceptMoreCoords { break }
					let nextCoord = parser.nextNArguments(1)
					if nextCoord.isEmpty { break }
					segments[segments.count - 1].addCoordinate(nextCoord)
					continue
				}
			}
